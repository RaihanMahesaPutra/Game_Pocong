var sceneMenu = new Phaser.Class({
    ...
    update(){
        // Manual boundary checking untuk semua enemy
    this.enemies.children.entries.forEach(enemy => {
        // Cek apakah enemy sudah mentok di atas
        if (enemy.y <= 0) {
            enemy.y = 0; // Set posisi tepat di boundary
            ...
            // Trigger efek ketika mentok
            this.onEnemyHitTop(enemy);
        }
        
        // Update posisi manual (jika tidak menggunakan physics)
...
onEnemyHitTop(enemy) {
    console.log('Enemy mentok di atas (manual detection)');
    
    // Efek visual
    enemy.setTint(0xff0000); // Berubah warna merah sementara
    
    // Reset tint setelah 200ms
...
// =====================================
// 5. SETUP DENGAN WORLD BOUNDS (Phaser Built-in)
// =====================================

setupEnemyWithWorldBounds() {
    const enemy = this.physics.add.sprite(400, 600, 'enemy_sprite');
    
    // Enable world bounds collision
    enemy.body.setCollideWorldBounds(true);
    enemy.body.setBounceY(0.8);
    enemy.body.setBounceX(0.5);
    
    // Set velocity
    enemy.body.setVelocityY(-200);
    
    // Event listener untuk world bounds collision
    enemy.body.onWorldBounds = true;
    this.physics.world.on('worldbounds', (event, body) => {
        if (body.gameObject === enemy) {
            console.log('Enemy hit world boundary!');
            
            // Cek apakah menabrak bagian atas
            if (body.y <= 0) {
                this.onEnemyHitTop(enemy);
            }
        }
...
// =====================================
// 6. MULTIPLE ENEMIES DENGAN SPAWNER
// =====================================
...
createEnemySpawner() {
    ...
    this.enemySpawner = this.time.addEvent({
        delay: 2000,
        callback: () => {
            const x = Phaser.Math.Between(50, game.canvas.width - 50);
            const y = game.canvas.height + 50;
            this.spawnEnemy(x, y);
        },
        loop: true
    });
...
// Cleanup enemies yang sudah keluar dari layar
cleanupEnemies() {
    this.enemies.children.entries.forEach(enemy => {
        if (enemy.y < -100 || enemy.y > game.canvas.height + 100) {
            enemy.destroy();
        }
    });
}
