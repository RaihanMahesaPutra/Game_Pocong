var sceneMenu = new Phaser.Class({
    ...
    update(){
        if (this.gameStarted && enemies) {
    enemies.children.entries.forEach(function(enemy) {
        // Cek jika enemy terlalu tinggi (di atas 50 pixel dari atas)
        if (enemy.y < 50) {
            enemy.setVelocityY(Math.abs(enemy.body.velocity.y)); // Paksa ke bawah
        }
        
        // Cek jika enemy terlalu rendah (di bawah 600 pixel)
        if (enemy.y > game.canvas.height - 100) {
            enemy.setVelocityY(-Math.abs(enemy.body.velocity.y)); // Paksa ke atas
        }
        
        // Cek batas kiri dan kanan
        if (enemy.x < 50) {
            enemy.setVelocityX(Math.abs(enemy.body.velocity.x)); // Paksa ke kanan
        }
        if (enemy.x > game.canvas.width - 50) {
            enemy.setVelocityX(-Math.abs(enemy.body.velocity.x)); // Paksa ke kiri
        }
    });
}
    }
