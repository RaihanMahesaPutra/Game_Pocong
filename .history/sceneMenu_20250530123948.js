var sceneMenu = new Phaser.Class({
    ...
    create(){
        ...
        const mummySprite = this.add.sprite(850, 768 / 2 + 145, "sps_mummy");
mummySprite.setScale(4);
mummySprite.setDepth(10);
mummySprite.play('mummy_walk');

// Menambahkan properti untuk pergerakan
mummySprite.velocityY = -2; // Kecepatan gerak ke atas (negatif = ke atas)
mummySprite.minY = 50; // Batas atas (jarak dari top)
mummySprite.maxY = 768 - 50; // Batas bawah
mummySprite.originalY = mummySprite.y; // Simpan posisi awal

// Menyimpan reference ke sprite untuk digunakan di update()
this.mummySprite = mummySprite;
    ...
    update(){
        if (this.mummySprite) {
        // Update posisi Y mummy
        this.mummySprite.y += this.mummySprite.velocityY;
        
        // Cek apakah sudah mentok di atas
        if (this.mummySprite.y <= this.mummySprite.minY) {
            this.mummySprite.y = this.mummySprite.minY;
            this.mummySprite.velocityY = Math.abs(this.mummySprite.velocityY); // Balik arah ke bawah
        }
        
        // Cek apakah sudah mentok di bawah
        if (this.mummySprite.y >= this.mummySprite.maxY) {
            this.mummySprite.y = this.mummySprite.maxY;
            this.mummySprite.velocityY = -Math.abs(this.mummySprite.velocityY); // Balik arah ke atas
        }
    }
}

// =====================================
// Solusi 3: Menggunakan Phaser Tween untuk pergerakan bolak-balik
// Tambahkan ini setelah membuat mummySprite (alternatif dari solusi 1 & 2)
...
// Animasi pergerakan bolak-balik menggunakan tween
this.tweens.add({
    targets: mummySprite,
    y: 50, // Posisi atas
...
// =====================================
// Solusi 4: Dengan physics body (jika menggunakan Arcade Physics)
// Tambahkan di bagian create() setelah membuat mummySprite

// Aktifkan physics untuk sprite
this.physics.add.existing(mummySprite);
mummySprite.body.setVelocityY(-100); // Kecepatan ke atas
mummySprite.body.setBounceY(1); // Bounce penuh ketika menabrak

// Buat invisible boundaries
const topBoundary = this.add.rectangle(game.canvas.width/2, 25, game.canvas.width, 50, 0x000000);
topBoundary.setVisible(false);
this.physics.add.existing(topBoundary, true); // Static body

const bottomBoundary = this.add.rectangle(game.canvas.width/2, 768-25, game.canvas.width, 50, 0x000000);
bottomBoundary.setVisible(false);
this.physics.add.existing(bottomBoundary, true); // Static body

// Collision detection
this.physics.add.collider(mummySprite, topBoundary);
this.physics.add.collider(mummySprite, bottomBoundary);
    }
});